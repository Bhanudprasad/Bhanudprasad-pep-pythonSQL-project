...
def load_and_clean_users(file_path):
    with open(file_path, 'r') as file:
        ...
        for row in reader:
            ...
            # Insert valid records into users table
...
def load_and_clean_call_logs(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for row in reader:
            if len(row) == 5 and all(row):  # Ensure we have all five columns and they are not empty
            ...
            elif len(row) == 6 and all(row[:5]): # Handle the extra comma in one row
                ...
                print(f"Skipping incomplete call log record: {row}")
    conn.commit()
